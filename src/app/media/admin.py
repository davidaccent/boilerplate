...
class ImageAdmin(ModelAdmin):
    ...
    delete_renditions_template = "media/admin/image_renditions_delete.html"
    ...
    @classmethod
    async def do_rendition_delete(cls):
    ...
    @classmethod
    async def delete_renditions_view(cls, request):
        if not has_required_scope(request, cls.permission_scopes):
            raise HTTPException(403)
        ...
        if request.method == "GET":
            ...
            return admin_config.templates.TemplateResponse(
                cls.delete_renditions_template, context
            )
    ...
    def url_names(cls):
        ...
        names.update(
            {"delete_renditions": f"{cls.site.name}:{mount}_delete_renditions"}
        )
        return names
    ...
    def routes(cls):
        ...
        routes.add_route(
            "/renditions/delete",
            endpoint=cls.delete_renditions_view,
            methods=["GET", "POST"],
            name=f"{mount}_delete_renditions",
        )
        return routes
